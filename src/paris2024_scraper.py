...
def get_paris_results(url):
    ...
    if response.status_code == 200:
        ...
        for block in sport_blocks:
            ...
            subcategories = block.find_all("div", class_="sport-calendar-cell js-jo-cell")
            for subcategory in subcategories:
                # get the subcategory, such as "Floor Exercise (M)"
                sub_cat_name = subcategory.find("div", class_="sport-calendar-cell__title").get_text(strip=True)
                medal_results[sport_name] = {sub_cat_name : []}
                
                class_participants = subcategory.find_all("div", class_="sport-participant")
                for class_participant in class_participants:
                    participant_name = class_participant.find("div", class_="sport-participant__name").get_text(strip=True)
                    medal_text = class_participant.find("div", class_="sport-participant__status")
                    span_tag = medal_text.find("span")
                    medal = span_tag["class"][0]
                    medal = medal.split("-")[-1]
                    # Find the img tag with class "sport-participant__img"
                    img_tag = class_participant.find("img", class_="sport-participant__img")
